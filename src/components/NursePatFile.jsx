...
import AddBillPharm from "./Accounting/AddBillPharm";
import AddBillLab from "./Accounting/AddBillLab";
import AddBill from "./Accounting/AddBill";
import Invoice from "./Accounting/Invoice";

const NursePatFile = forwardRef(({ patient }, ref) => {
  const { setCurrentPat, appointments, filteredPatients, patStats } =
    usePatientData();
  const { currentStaff } = useAuth();
  const { register, handleSubmit, addVitals } = useNurseData();

  const [showApp, setShowApp] = useState(false);
  const [showAddBill, setShowAddBill] = useState(false);
  const [showInv, setShowInv] = useState(false);
  const [addPatVitals, setAddPatVitals] = useState(false);
  ...
  return (
    ...
    >
      ...
      <div className="flex space-x-8">
        ...
        <button
          className="mt-6 w-full rounded-xl bg-blue-500 px-6 py-3 text-xl font-semibold text-white transition-all hover:bg-blue-700"
          onClick={() => setAddPatVitals(!addPatVitals)}
        >
          Add Vitals
        </button>
      </div>
      ...
      {addPatVitals && (
        <div className="my-12 flex justify-center">
          <div className="mt-6 w-5/6 rounded-lg bg-white p-6 shadow-md">
            <h2 className="mb-6 text-2xl font-bold text-blue-800">
              Check Patient Vitals
            </h2>
            <form
              onSubmit={handleSubmit((data) => addVitals(data, patientId))}
              className="flex flex-col items-center justify-center space-y-6"
            >
              {/* Input Fields */}
              <input
                type="text"
                className="input-field rounded-lg border p-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300"
              ...
              <input
                ...
                {...register("patHeartRate")}
              />
              <input
                type="text"
                className="input-field rounded-lg border p-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-300"
                placeholder="Blood Pressure"
                {...register("patBloodPressure")}
              />

              {/* Submit Button */}
              <button
                type="submit"
                className="mt-4 w-full rounded-2xl bg-green-500 py-3 font-semibold text-white transition duration-300 hover:bg-green-600"
              >
                Submit Vitals
              </button>
            </form>
...
NursePatFile.displayName = "NursePatFile";

export default NursePatFile;
